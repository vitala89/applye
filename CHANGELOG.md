...
- **Page size & margins for CV and cover letter.** Both editors gained a Page size (A4 / Letter) and Margins (Narrow / Normal / Wide) control; the preview reflects the margin padding, and the choice is applied to DOCX (section page size + margins) and PDF (page dimensions, margins, and wrap width) export. Stored in `style_json`, defaulting to A4 / normal for existing documents.
