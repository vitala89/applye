...
- **The documentation shows the real app instead of dashed boxes, on eight screens so far.** `guide/settings-ai.png` (API mode with Anthropic, the full privacy note, the API key block in its stored state), `guide/onboarding.png` (the AI step with both modes and the skip warning), `guide/sidebar.png` (the whole sidebar against a working Dashboard), `guide/profile-filled.png` (the contact block, two expanded positions, skills and languages), `guide/profile-archetypes.png` (three target roles, one per fit tier), `guide/score-result.png` (a genuine 72 per cent scoring run with its missing keyword, ATS check and three red flags) `guide/my-jobs-table.png` (every status and every legitimacy tier, two rows carrying real scores) and `guide/analytics.png` (the counters, the funnel and the weekly chart). Every one is captured from the running desktop app in the dark theme against a seeded demo profile of invented companies; nothing is drawn or retouched, and no key, contact or real employer appears in any frame. Seventeen placeholders remain, tracked in `docs/product/MEDIA_SHOTLIST.md`.
