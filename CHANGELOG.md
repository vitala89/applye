...
- **You can change a job's status straight from the job page.** The detail page now has a status dropdown, so you can move a job to Interview, Offer, Rejected - or back - without going to the pipeline board. It writes the same way the board does, and the job list stays in sync.
