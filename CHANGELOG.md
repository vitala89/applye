...
- **The documentation shows the real app instead of dashed boxes, on five screens so far.** `guide/settings-ai.png` (API mode with Anthropic, the full privacy note, the API key block in its stored state), `guide/onboarding.png` (the AI step with both modes and the skip warning), `guide/sidebar.png` (the whole sidebar against a working Dashboard), `guide/profile-filled.png` (the contact block, two expanded positions, skills and languages) and `guide/profile-archetypes.png` (three target roles, one per fit tier). Every one is captured from the running desktop app in the dark theme against a seeded demo profile of invented companies; nothing is drawn or retouched, and no key, contact or real employer appears in any frame. Twenty placeholders remain, tracked in `docs/product/MEDIA_SHOTLIST.md`.
