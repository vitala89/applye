...
**[Скачать для macOS (Apple Silicon)](https://github.com/vitala89/applye/releases/latest)** - `.dmg`, 15 МБ.

Сборки для Windows, Linux и macOS на Intel пока не опубликованы: их собирает релизный workflow,
который не может запуститься, пока не решён вопрос с оплатой GitHub Actions для этого репозитория.
До этого - [сборка из исходников](#быстрый-старт), три команды, работает на всех трёх платформах.

> Сборка для macOS не подписана сертификатом Apple Developer, поэтому первый запуск нужно делать
> правым кликом по приложению и **Открыть**, а не двойным кликом. В
> [примечаниях к релизу](https://github.com/vitala89/applye/releases/latest) объяснено почему и дана альтернатива одной строкой.
