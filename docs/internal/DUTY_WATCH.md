...
### 2026-07-28 (latest), two capture-session findings fixed, one of them found to be misreported, and four described-but-absent features parked on purpose

- **Status:** partial - the code and the decisions are finished, the native visual gate is not
- **Agent/tool:** Claude Code (Opus)
- **Branch:** `fix/prelaunch-capture-findings`
- **Commits:** `a249fad`, `90068e0`, `771cc55`, `eb412e4`, plus the docs commit carrying this entry
...
- **Next first action:** Stop whatever holds port 4200 (`lsof -nP -iTCP:4200 -sTCP:LISTEN`), run `npm run desktop:dev` on this branch, open Profile and type "Senior Engineer" as a target role to confirm the warning appears and reads correctly in both themes, then open Interview Prep and confirm the row menu's new first entry and the Space key on a row.
