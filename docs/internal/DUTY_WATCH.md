...
- **Next first action:** the reporter runs `claude` in a terminal, signs in, and re-runs onboarding on this branch to confirm a resume actually parses. That is the last unverified step - no import has yet been observed succeeding on this branch, only failing correctly.
