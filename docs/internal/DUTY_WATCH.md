...
- **Next first action:** Open `http://localhost:4300/docs/guide/discover` in a real browser and look at all seven guide pages by eye, in both themes; then, if they read correctly, flip `SEARCH_INDEXABLE` and the `X-Robots-Tag` line together and deploy.
