...
import type { CvContent, CvParsedContent, CvSection, DocumentLibraryItem } from '@applye/core';
...
import { CvRegenerationStore, type CvRegenerationCodec } from './cv-regeneration.store';
...
describe('CvRegenerationStore', () => {
  ...
  beforeEach(() => {
    ...
    codec = {
      ...
      mergeSection: jest.fn((content: CvContent): CvContent => ({
        sections: [...content.sections, sec('regenerated', 9)],
      })),
    };
  ...
  describe('regenerateSection', () => {
    it('renders the skill with the document context and folds the answer back in', async () => {
      await expect(store.regenerateSection('summary', codec)).resolves.toBe(true);
    ...
    it('reads settings once, and passes them to the run', async () => {
      await store.regenerateSection('summary', codec);
    ...
    it('skips the model when the section hash already matches', async () => {
      ...
      await expect(store.regenerateSection('summary', codec)).resolves.toBe(false);
    ...
    it('raises a typed error for a profile with no markdown, and clears the flag', async () => {
      ...
      await expect(store.regenerateSection('summary', codec)).rejects.toBeInstanceOf(
        CvNoProfileError,
      );
    ...
    it('clears the in-flight key when the model call fails', async () => {
      ...
      await expect(store.regenerateSection('summary', codec)).rejects.toThrow('rate limited');
      expect(store.regeneratingKey()).toBeNull();
    });
  });
  ...
  describe('pullFromProfile', () => {
    it('merges fresh personal details into a NEW section object', async () => {
      ...
      await expect(store.pullFromProfile(codec)).resolves.toBe(true);
    ...
    it('leaves the other sections untouched', async () => {
      await store.pullFromProfile(codec);
    ...
    it('refuses when the document has no personal-details section', async () => {
      ...
      await expect(store.pullFromProfile(codec)).resolves.toBe(false);
      expect(ai.run).not.toHaveBeenCalled();
    });
    ...
    it('raises a typed error for a profile with no markdown, and clears the flag', async () => {
      ...
      await expect(store.pullFromProfile(codec)).rejects.toBeInstanceOf(CvNoProfileError);
